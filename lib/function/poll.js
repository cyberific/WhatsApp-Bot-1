...
async function voteadapter(piyo, message, pollfile, voterslistfile) {
    console.log('flag1')
        //voteadapter
...
function addvote(piyo, message, num, pollfile, voterslistfile) {
    ...
    op = op + ls;
    op = op + `\n Untuk memvote silakan ketik ${botPrefix}addvote <number> \n [Contoh *${botPrefix}addvote 2*]`;
...
async function adminpollreset(piyo, message, polltitle, pollfile, voterslistfile) {
    if (await isGroupAdmin(piyo, message, message.author)) {
        ...
        piyo.reply(message.chatId, `*🎉️ *Berhasil membuat poll\n\n 🎯️ *Title : ${polltitle}*\n \n⭐️ gunakan ${botPrefix}addv <kandidat> untuk menambahkan kandidat`, message.id);
        //voterresetter
        let data = {
            list: ['testentry']
        }
